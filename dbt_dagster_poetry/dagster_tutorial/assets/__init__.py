...
from dagster_tutorial import AssetIn, MetadataValue, asset, file_relative_path


# 这里的 key_prefix 和后面引入 dbt 项目给定的 prefix 一致
@asset(key_prefix=["dbt_jaffle_shop"], group_name="staging")
...
# DBT 项目的配置文件的位置
DBT_PROFILES = file_relative_path(__file__, "../../dbt_jaffle_shop/config")

# 这是一种新的上游定义的方式，通过 `ins` 关键字
# 并且设置前缀为 `dbt_jaffle_shop`，组为 `staging`，这样和前面的 asset 保持一致
@asset(
    ins={"customers": AssetIn(key_prefix=["dbt_jaffle_shop"])},
    group_name="staging",
)
...
dbt_assets = load_assets_from_dbt_project(
    project_dir=DBT_PROJECT_PATH, profiles_dir=DBT_PROFILES, key_prefix=["dbt_jaffle_shop"]
)