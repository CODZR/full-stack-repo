...
SELECT
      ...
      DATE_TRUNC(
            'Year',
            date_value
      ) - 1 + INTERVAL 1 YEAR AS last_day_of_year,
      ROW_NUMBER() over (PARTITION BY YEAR(date_value), MONTH(date_value), dayofweek(date_value)
ORDER BY
      date_value) AS ordinal_weekday_of_month
FROM
      generate_date
