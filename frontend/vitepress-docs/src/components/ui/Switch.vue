...
const isOpen = computed(() => (props.modelValue === props.activeValue));

const computedBackground  = computed(() => {
  // 若传递了激活颜色和未激活颜色，就根据是否开启状态使用传递的颜色
  if ((props.activeColor.length > 0) & (props.inactiveColor.length > 0)) {
    return isOpen.value ? props.activeColor : props.inactiveColor;
  }
  // 没传递就根据开启使用默认的背景色
  else {
    return isOpen.value ? '#409EFF' : '#C0CCDA';
  }
});
const changeStatus = () => {
  // 禁用情况下，不做状态更改切换
  ...
  const val = isOpen.value ? props.inactiveValue : props.activeValue;
  emit('update:modelValue', val); // 更新外层v-model绑定的值
  emit('change:modelValue', val); // 抛出一个change事件以供用户使用
};
</script>
