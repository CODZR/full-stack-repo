...
def test_create_faq(db: Session) -> None:
    ...
    faq = crud.faq.create_with_user(db=db, obj_in=faq_in, user_id=user.id)
    assert faq.title == title
    assert faq.description == description
    assert faq.user_id == user.id
...
def test_get_faq(db: Session) -> None:
    ...
    assert faq.user_id == stored_faq.user_id
...
def test_update_faq(db: Session) -> None:
    ...
    assert faq.user_id == faq2.user_id
