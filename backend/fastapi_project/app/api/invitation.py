...
def get_invitations(
    db: Session = Depends(get_db), current_user=Depends(get_current_user_authorization)
...
async def accept_invitation(token: str, db: Session = Depends(get_db)):
...
async def resend_invitation(
    ...
    if not invitation:
        raise HTTPException(
            status_code=400, detail="No existing invitation found for this email."
        )
